...
// https://leetcode.com/problems/falling-squares/discuss/2701495/c-solution-using-segment-tree-lazy-propagation-and-coordinate-compression
// Below lazy propagation implements range update and range query.
template<typename T> inline T Max(T a, T b){return a>b?a:b;}
template<typename T> inline T Min(T a, T b){return a<b?a:b;}
template<typename T> inline T __Gcd(T a, T b){ return b==0 ? a : __Gcd(b,a%b);}
template<typename T> struct SegTree {
    ...
    void update_range(int node, int st, int en, int l, int r, T x) {
        ...
        seg[node]=max(seg[node], max(lazy[2*node+1], lazy[2*node+2]));
    }
    ...
    T lazy_range_query(int l, int r) {
        return _lazy_query(0, l, r, 0, N-1);
    }
    
    T _lazy_query(int node, int l, int r, int st, int en) {
        if(l>en||r<st || st>en)return 0;
        if(st>=l && en<=r) {
            return max(seg[node], lazy[node]);
        }
        int mid = (st+en)/2;
        lazy[2*node+1]=max(lazy[2*node+1], lazy[node]);
        lazy[2*node+2]=max(lazy[node], lazy[2*node+2]);
        return max(_lazy_query(2*node+1, l, r, st, mid), _lazy_query(2*node+2, l, r, mid+1, en));
    }
};
